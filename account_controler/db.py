...
### Proxy
    ...
    def account_add(self,session,status,proxy_id,api_id,api_hash,first_name,last_name,chats,owner_id):
        self.cursor.execute(f'''INSERT INTO account (session,status,proxy_id,last_update,api_id,api_hash,first_name,last_name,chats,owner_id) VALUES (
        "{session}","{status}",{proxy_id},NOW(),{api_id},"{api_hash}","{first_name}","{last_name}",{chats},{owner_id}
        )''')

        self.connection.commit()

    def update_account(self,session,owner_id,first_name,last_name,chats):
        self.cursor.execute(f'''UPDATE account SET first_name="{first_name}",last_name="{last_name}",chats={chats} WHERE owner_id={owner_id} AND session="{session}"''')
        self.connection.commit()
    ...
    def get_user_accounts(self,owner_id):
        self.cursor.execute(f'SELECT session FROM account WHERE owner_id = "{owner_id}"')
        return self.cursor.fetchall()
