...
class DB:
    ...
    def create_tables(self):
        commansd = [
            ...
            "CREATE TABLE IF NOT EXISTS chats(id int AUTO_INCREMENT PRIMARY KEY, chats_id INT, link VARCHAR(50));",
            ...
            "CREATE TABLE IF NOT EXISTS account(session VARCHAR(32), status VARCHAR(10), project_id INT,proxy_id INT, last_update DATETIME, api_id INT, api_hash VARCHAR(32));",
...
### Proxy
    ...
    def account_add(self,session,status,project_id,proxy_id,api_id,api_hash):
        self.cursor.execute(f'''INSERT INTO account (session,status,project_id,proxy_id,last_update,api_id,api_hash) VALUES (
        "{session}","{status}",{project_id},{proxy_id},NOW(),{api_id},"{api_hash}"
        )''')
